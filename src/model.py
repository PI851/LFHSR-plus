...
class mask_part(nn.Module):
    def __init__(self, num_layers, in_channels, out_channels=1, scale=None, channel=32):
        ...
        if scale is None:
            layers.append(nn.Conv2d(in_channels=channel, out_channels=out_channels, kernel_size=3, padding=1,
                                    stride=1))
        else:
...
class LFHSR_mask(nn.Module):
    def __init__(self, scale, an, mask_num_layers=6, mask_channel=32, fusion_num_layers=6, fusion_channel=32,
        ...
        :param scale: 上采样比例因子
        :param an: 角度分辨率
        :param mask_num_layers:
        :param mask_channel:
        :param fusion_num_layers:
        :param fusion_channel:
        :param u_net_num_layers:
        :param u_net_channel:
        :param up_num_layers:
        :param up_channel:
        '''
    ...
    def warp_reverse_all(self, img, disparity_list, view_n, view_position):
    ...
    def forward(self, lr_LF_shear, lr_LF, hr_img, view_position, disparity_list):
        ...
        hr_img_deshear = self.warp_reverse_all(hr_img, -disparity_list, self.an, view_position)
        ...
        # TODO 在这里提前计算mask，优化fusion的输入，这里还需要进行校正，之后再补充
        hr_mask_view = 1
        for i in range(len(disparity_list)):
            hr_mask_view *= (1 - hr_mask_deshear[:, :, i])
        hr_mask_view = 1 - hr_mask_view

        hr_fusion_LF = hr_fusion_LF.reshape(-1, 1, self.scale * X, self.scale * Y)

        hr_LF *= hr_mask_view[:, 0]
